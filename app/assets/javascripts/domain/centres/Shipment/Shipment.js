...
function ShipmentFactory($q,
  ...
  const SCHEMA = {
    ...
    'properties': {
      ...
      'fromLocationInfo': { '$ref': 'CentreLocationInfo' },
      'toLocationInfo':   { '$ref': 'CentreLocationInfo' },
  ...
  class Shipment extends ConcurrencySafeEntity {
      ...
      super(Object.assign(
        {
          ...
          courierName: '',
          trackingNumber: '',
          fromLocationInfo: undefined,
          toLocationInfo: undefined
        },
        obj))
    }
    ...
    update(url, additionalJson) {
      return super.update(url, additionalJson).then(Shipment.asyncCreate);
    }
     ...
     */
    updateCourierName(courierName) {
      return this.update(Shipment.url('courier', this.id), { courierName: courierName });
    }
    ...
    updateTrackingNumber(trackingNumber) {
      return this.update(Shipment.url('trackingnumber', this.id), { trackingNumber: trackingNumber });
    }
     ...
     */
    updateFromLocation(fromLocation) {
      return this.update(Shipment.url('fromlocation', this.id), { locationId: fromLocation });
    }
     ...
     */
    updateToLocation(toLocation) {
      return this.update(Shipment.url('tolocation', this.id), { locationId: toLocation });
    }
     ...
     */
    created() {
      return this.update(Shipment.url('state/created', this.id));
    }
     ...
     */
    pack(datetime) {
      return this.update(Shipment.url('state/packed', this.id), { datetime: datetime });
    }
     ...
     */
    send(datetime) {
      return this.update(Shipment.url('state/sent', this.id), { datetime: datetime });
    }
     ...
     */
    receive(datetime) {
      return this.update(Shipment.url('state/received', this.id), { datetime: datetime });
    }
     ...
     */
    unpack(datetime) {
      return this.update(Shipment.url('state/unpacked', this.id), { datetime: datetime });
    }
     ...
     */
    complete(datetime) {
      return this.update(Shipment.url('state/completed', this.id), { datetime: datetime });
    }
     ...
     */
    lost() {
      return this.update(Shipment.url('state/lost', this.id));
    }
     ...
     * @returns {Promise} A copy of this shipment, but with the state set to SENT.
     */
    skipToStateSent(timePacked, timeSent) {
      var json = { timePacked: timePacked, timeSent: timeSent };
      return this.update(Shipment.url('state/skip-to-sent', this.id), json);
    }
     ...
     * @returns {Promise} A copy of this shipment, but with the state set to UNPACKED.
     */
    skipToStateUnpacked(timeReceived, timeUnpacked) {
      var json = { timeReceived: timeReceived, timeUnpacked: timeUnpacked };
      return this.update(Shipment.url('state/skip-to-unpacked', this.id), json);
    }
     ...
     */
    canAddInventoryId(specimenInventoryId) {
     ...
     */
    addSpecimens(specimenInventoryIds, shipmentContainerId) {
     ...
     * @returns {Promise} A copy of this shipment.
     */
    updateShipmentContainerOnSpecimens(shipmentSpecimens, shipmentContainerId) {
      var reqJson;
    ...
    /** @private */
    tagSpecimens(inventoryIds, urlPath) {
     ...
     * @returns {Promise} A copy of this shipment.
     */
    tagSpecimensAsPresent(inventoryIds) {
      return this.tagSpecimens(inventoryIds, ShipmentItemState.PRESENT);
    }
     ...
     * @returns {Promise} A copy of this shipment.
     */
    tagSpecimensAsReceived(inventoryIds) {
      return this.tagSpecimens(inventoryIds, ShipmentItemState.RECEIVED);
    }
     ...
     * @returns {Promise} A copy of this shipment.
     */
    tagSpecimensAsMissing(inventoryIds) {
      return this.tagSpecimens(inventoryIds, ShipmentItemState.MISSING);
    }
    ...
    tagSpecimensAsExtra(inventoryIds) {
      return this.tagSpecimens(inventoryIds, ShipmentItemState.EXTRA);
    }
  }
...
export default ngModule => ngModule.factory('Shipment', ShipmentFactory)
