 ...
 */
export default function entityUpdateSharedSpec(context) {
  ...
  describe('update functions', function () {
    ...
    it('should update a field on the enitity', function() {
      spyOn(context.entity.prototype, context.updateFuncName)
        .and.returnValue(this.$q.when(context.updateReturnValue));
    ...
    it('should display an error in a modal when update fails', function() {
      ...
      this.modalInput[context.modalInputFuncName] = jasmine.createSpy()
        .and.returnValue({ result: this.$q.when(newValue) });
