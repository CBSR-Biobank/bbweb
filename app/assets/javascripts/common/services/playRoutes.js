...
define(["angular", "require", "jsRoutes"], function(angular, require, jsRoutes) {
  ...
  mod.service("playRoutes", ["$http", function($http) {
    ...
    // FIXME: this needs to be fixed so that controller sub packages are configured
    // correctly
    var convertPlayRoutes = function(packageName, playRoutes) {
      // Loop over all items in the jsRoutes generated by Play, wrap and add them to playRoutes
      for (var packageKey in packageName) {
        var packageObject = packageName[packageKey];

        //console.log("packageKey: " + packageKey + ", packageObject: " + packageObject);

        addPackageObject(packageKey, playRoutes);
        for (var controllerKey in packageObject) {
          var controller = packageObject[controllerKey];

          //console.log("controllerKey: " + controllerKey + ", controller: " + controller);

          addControllerObject(packageKey, controllerKey, playRoutes);
          for (var controllerMethodKey in controller) {
            //console.log("controllerMethodKey: " + controllerMethodKey);
    ...
    // correctly
    var playRoutes = {};
    convertPlayRoutes(jsRoutes.org.biobank, playRoutes);
    convertPlayRoutes(jsRoutes.org.biobank.controllers, playRoutes);
    return playRoutes;
